...
load(['Mapping_Refugee_MLE_Model=' num2str(AIC_model_ref) '.mat'],'par_V');

load('MCMC_Sample_Forcible_Displacement_Parameters.mat','Parameter_Samp','L_V_Samp');

NS=length(L_V_Samp)/4;
Parameter_Samp_Ref=zeros(length(L_V_Samp),k(daics==0));
L_V_Samp_Ref=zeros(size(L_V_Samp));

[LB,UB]=ParameterBounds_Mapping_Refugee(AIC_model_ref);

options = optimoptions('ga','UseParallel',true,'InitialPopulationMatrix',par_V);
...
for ii=1:NS
    ...
    [Parameter_Samp_Ref(ii,:),fval]=ga(@(x)ObjectiveFunction_Refugee(x,Daily_Refugee,Mapping_Data,Refugee_Displacement,Number_Displacement,AIC_model_ref),length(LB),[],[],[],[],LB,UB,[],options);
    L_V_Samp_Ref(ii)=-fval;    
end

save('MCMC_Sample_Refugee_Mapping_Parameters_1.mat','Parameter_Samp_Ref','L_V_Samp_Ref');
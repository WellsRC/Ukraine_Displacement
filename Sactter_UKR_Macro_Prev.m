...
for ii=1:5
   ...
   subplot('Position',[0.168432203389831,0.221476510067114,0.82156779661017,0.664429530201342])
    
    Prev_PW=1000.*Pre_War(ii,:)./Pop_PW';
    
    Prev_IDP=1000.*IDP_Disease(ii,:)./IDP_Pop_FD';
    Prev_non_IDP=1000.*Non_IDP_Disease(ii,:)./non_IDP_Pop_FD';
    Prev_FD=1000.*(IDP_Disease(ii,:)+Non_IDP_Disease(ii,:))./(IDP_Pop_FD'+non_IDP_Pop_FD');
    
    
    Prev_PW=Prev_PW([1 2 3 5 6 7]);
    Prev_IDP=Prev_IDP([1 2 3 5 6 7]);
    Prev_non_IDP=Prev_non_IDP([1 2 3 5 6 7]);
    Prev_FD=Prev_FD([1 2 3 5 6 7]);
    
    mmin=min([Prev_PW(:);Prev_IDP(:);Prev_non_IDP(:);Prev_FD(:)]);
    mmax=max([Prev_PW(:);Prev_IDP(:);Prev_non_IDP(:);Prev_FD(:)]);
    
    p1=scatter(Prev_PW,Prev_IDP,90,CC(ii,:),'s','LineWidth',2); hold on;
    p2=scatter(Prev_PW,Prev_non_IDP,90,CC(ii,:),'d','LineWidth',2);
    p3=scatter(Prev_PW,Prev_FD,90,CC(ii,:),'filled');
    plot(linspace(mmin*0.95,mmax.*1.05,101),linspace(mmin*0.95,mmax.*1.05,101),'k-','LineWidth',2);
    box off;
    set(gca,'LineWidth',2,'tickdir','out','Fontsize',22,'Xminortick','on','Yminortick','on');
    xlabel('Cases per 1,000 (Pre-invasion)','Fontsize',28);
    ylabel({'Cases per 1,000','(Post-invasion)'},'Fontsize',28);
    title(Disease{ii},'Fontsize',30);
    legend([p1 p2 p3],{'IDP','non-IDP','IDP and non-IDP'},'Fontsize',18,'Location','NorthWest')
    xlim([mmin*0.95 mmax.*1.05]);
    ylim([mmin*0.95 mmax.*1.05]);
    text(mmin*0.95-0.1981.*(mmax.*1.05-mmin*0.95),mmin*0.95+1.078.*(mmax.*1.05-mmin*0.95),char(65+ii),'Fontsize',40,'FontWeight','bold');
    print(gcf,['Scatter_Prev_Force_Disp_' Disease{ii} '.png'],'-dpng','-r300');
end

...
NS=length(L_V_Samp);
Parameter_Samp_IDP=zeros(NS,k(daics==0));
...
for ii=1:NS
    ...
    [Parameter_Samp_IDP(ii,:),fval]=surrogateopt(@(x)ObjectiveFunction_IDP(x,Daily_IDP_Origin,Mapping_Data,IDP_Displacement,Time_Sim,Parameter,Shapefile_Raion_Name,Shapefile_Raion_Oblast_Name,Shapefile_Oblast_Name,Macro_Map,model_num-1),LB,UB,options);
    L_V_Samp_IDP(ii)=-fval;    
end

save('MCMC_Sample_IDP_Mapping_Parameters.mat','Parameter_Samp_IDP','L_V_Samp_IDP');